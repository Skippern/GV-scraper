...
logging.basicConfig(filename="/var/log/GTFS/expressa.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('expressa.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/expressa.json') as infile:
...
durationsList[u"operator"] = u"Expressa Transportes Ltda"
durationsList[u"network"] = u"Expressa"
