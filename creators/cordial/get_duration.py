...
logging.basicConfig(filename="/var/log/GTFS/cordial.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('cordial.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/cordial.json') as infile:
...
durationsList[u"operator"] = u"Cordial Transportes e Turismo Ltda"
durationsList[u"network"] = u"Cordial"
