...
logging.basicConfig(filename="/var/log/GTFS/sao_marcos.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('sao_marcos.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/sao_marcos.json') as infile:
...
durationsList[u"operator"] = u"Expresso São Marcos Ltda"
durationsList[u"network"] = u"Expresso São Marcos"
