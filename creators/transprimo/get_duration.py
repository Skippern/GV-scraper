...
logging.basicConfig(filename="/var/log/GTFS/transprimo.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('transprimo.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/transprimo.json') as infile:
...
durationsList[u"operator"] = u"Transprimo Ltda"
durationsList[u"network"] = u"Transprimo"
