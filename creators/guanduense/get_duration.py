...
logging.basicConfig(filename="/var/log/GTFS/guanduense.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('guanduense.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/guanduense.json') as infile:
...
durationsList[u"operator"] = u"Viação Guanduense Ltda"
durationsList[u"network"] = u"Guanduense"
