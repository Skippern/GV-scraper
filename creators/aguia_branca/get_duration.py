...
logging.basicConfig(filename="/var/log/GTFS/aguia_branca.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('aguia_branca.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/aguia_branca.json') as infile:
...
durationsList[u"operator"] = u"Viação Águia Branca"
durationsList[u"network"] = u"Águia Branca"
