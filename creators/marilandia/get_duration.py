...
logging.basicConfig(filename="/var/log/GTFS/marilandia.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('marilandia.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/marilandia.json') as infile:
...
durationsList[u"operator"] = u"Viação Marilândia Ltda"
durationsList[u"network"] = u"Viação Marilândia"
