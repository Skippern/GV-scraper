...
logging.basicConfig(filename="/var/log/GTFS/joana_darc.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('joana_darc.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/joana_darc.json') as infile:
...
durationsList[u"operator"] = u"Viação Joana Dárc S/A"
durationsList[u"network"] = u"Joana Dárc"
