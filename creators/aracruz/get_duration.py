...
logging.basicConfig(filename="/var/log/GTFS/aracruz.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('aracruz.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/aracruz.json') as infile:
...
durationsList[u"operator"] = u"Expresso Aracruz Ltda"
durationsList[u"network"] = u"Expresso Aracruz"
