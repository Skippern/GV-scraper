...
logging.basicConfig(filename="/var/log/GTFS/sao_goncalo.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('sao_goncalo.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/sao_goncalo.json') as infile:
...
durationsList[u"operator"] = u"Viação São Gonçalo Ltda"
durationsList[u"network"] = u"Viação São Gonçalo"
