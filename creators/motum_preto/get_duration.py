...
logging.basicConfig(filename="/var/log/GTFS/motum_preto.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('motum_preto.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/motum_preto.json') as infile:
...
durationsList[u"operator"] = u"Viação Motum Preto Ltda"
durationsList[u"network"] = u"Viação Motum Preto"
