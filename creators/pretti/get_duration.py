...
logging.basicConfig(filename="/var/log/GTFS/pretti.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('pretti.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/pretti.json') as infile:
...
durationsList[u"operator"] = u"Viação Pretti Ltda"
durationsList[u"network"] = u"Viação Pretti"
