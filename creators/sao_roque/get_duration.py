...
logging.basicConfig(filename="/var/log/GTFS/sao_roque.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('sao_roque.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/sao_roque.json') as infile:
...
durationsList[u"operator"] = u"Viação São Roque Ltda"
durationsList[u"network"] = u"Viação São Roque"
