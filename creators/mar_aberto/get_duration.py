...
logging.basicConfig(filename="/var/log/GTFS/mar_aberto.log", level=logging.DEBUG, format="%(asctime)s %(name)s %(levelname)s - %(message)s", datefmt="%Y/%m/%d %H:%M:%S:")
...
config = {}
with open('mar_aberto.json', 'r') as infile:
    config = json.load(infile)

source = {}
with open('../../sources/der-es/mar_aberto.json') as infile:
...
durationsList[u"operator"] = u"Viação Mar Aberto Ltda"
durationsList[u"network"] = u"Viação Mar Aberto"
